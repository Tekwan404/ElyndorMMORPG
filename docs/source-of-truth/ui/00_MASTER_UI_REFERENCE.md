...
These are current paired screen boards. The earlier individual `references/PRIMARY_VISUAL_CANON` files and master board were removed from the repository.
...
Structure is defined by `UI_05` and the current `reference/UI_05-06_STATS_TALENTS.png` board. Historical `STRUCTURE_ONLY` assets are unavailable and do not define the Elyndor style.
...
4. использовать актуальную board из `reference/`;
5. не изобретать gameplay;
6. после утверждения добавляться в `reference/` с обновлением `docs/source-of-truth/ui/00_UI_REFERENCE_INDEX.md`.
