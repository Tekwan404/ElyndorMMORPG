# Elyndor — MASTER Project Index

**Package:** MASTER Source of Truth v7.1
**Purpose:** точка входа в полный проектный пакет.

---

# 1. Как пользоваться пакетом

```text
README.md
→ docs/source-of-truth/00_README_SOURCE_OF_TRUTH.md
→ docs/source-of-truth/00_MASTER_PROJECT_INDEX.md
→ нужная system specification
→ нужная UI specification
→ docs/source-of-truth/ui/00_MASTER_UI_REFERENCE.md
→ visual reference PNG
```

---

# 2. Gameplay / System Source of Truth

- `docs/source-of-truth/gameplay/01_TIME_SYSTEM.md`
- `docs/source-of-truth/gameplay/02_COMBAT_SYSTEM.md`
- `docs/source-of-truth/gameplay/03_AFK_FARMING_SYSTEM.md`
- `docs/source-of-truth/gameplay/04_WORLD_AND_LOCATIONS_SYSTEM.md`
- `docs/source-of-truth/gameplay/05_CHARACTER_SYSTEM.md`
- `docs/source-of-truth/gameplay/06_ATTRIBUTES_AND_STATS_SYSTEM.md`
- `docs/source-of-truth/gameplay/07_RESOURCE_SYSTEM.md`
- `docs/source-of-truth/gameplay/08_EFFECT_SYSTEM.md`
- `docs/source-of-truth/gameplay/09_DAMAGE_AND_HEALING_SYSTEM.md`
- `docs/source-of-truth/gameplay/10_ABILITY_SYSTEM.md`
- `docs/source-of-truth/gameplay/11_PROGRESSION_SYSTEM.md`
- `docs/source-of-truth/gameplay/12_CLASS_SYSTEM.md`
- `docs/source-of-truth/gameplay/13_ITEM_EQUIPMENT_SYSTEM.md`
- `docs/source-of-truth/gameplay/14_LOOT_SYSTEM.md`
- `docs/source-of-truth/gameplay/15_MONSTER_AND_AI_SYSTEM.md`
- `docs/source-of-truth/gameplay/16_TALENT_SYSTEM.md`
- `docs/source-of-truth/gameplay/17_QUEST_SYSTEM.md`
- `docs/source-of-truth/gameplay/18_BOSS_AND_WORLD_EVENT_SYSTEM.md`
- `docs/source-of-truth/gameplay/19_CLASS_ROSTER_AND_CHARACTER_CREATION.md`
- `docs/source-of-truth/gameplay/20_PARTY_SYSTEM.md`
- `docs/source-of-truth/gameplay/21_COMPANION_AND_PET_SYSTEM.md`
- `docs/source-of-truth/gameplay/22_WARRIOR_TALENT_TREE.md`
- `docs/source-of-truth/gameplay/23_ARCHER_TALENT_TREE.md`
- `docs/source-of-truth/gameplay/24_EQUIPMENT_SETS_LEVEL_5_30.md`
- `docs/source-of-truth/gameplay/25_MAGE_TALENT_TREE.md`
- `docs/source-of-truth/gameplay/26_CURRENCY_AND_ECONOMY_SYSTEM.md`
- `docs/source-of-truth/gameplay/27_TRADE_AND_AUCTION_SYSTEM.md`
- `docs/source-of-truth/gameplay/28_DUNGEON_SYSTEM.md`
- `docs/source-of-truth/gameplay/29_CRAFTING_AND_PROFESSION_SYSTEM.md`
- `docs/source-of-truth/gameplay/30_GUILD_SYSTEM.md`
- `docs/source-of-truth/gameplay/31_RAID_GROUP_SYSTEM.md`

---

# 3. UI/UX Specifications

- `docs/source-of-truth/ui/UI_01_GLOBAL_GAME_SHELL.md`
- `docs/source-of-truth/ui/UI_02_WORLD_AND_LOCATION.md`
- `docs/source-of-truth/ui/UI_03_HERO.md`
- `docs/source-of-truth/ui/UI_04_INVENTORY_AND_ITEMS.md`
- `docs/source-of-truth/ui/UI_05_CHARACTER_STATS.md`
- `docs/source-of-truth/ui/UI_06_TALENTS.md`
- `docs/source-of-truth/ui/UI_07_COMPANION.md`
- `docs/source-of-truth/ui/UI_08_NORMAL_COMBAT.md`
- `docs/source-of-truth/ui/UI_09_WORLD_BOSS_COMBAT.md`
- `docs/source-of-truth/ui/UI_10_PARTY.md`
- `docs/source-of-truth/ui/UI_11_QUESTS.md`
- `docs/source-of-truth/ui/UI_12_CITY_LOCATION.md`
- `docs/source-of-truth/ui/UI_13_MERCHANT.md`
- `docs/source-of-truth/ui/UI_14_AUCTION.md`
- `docs/source-of-truth/ui/UI_15_DUNGEON.md`
- `docs/source-of-truth/ui/UI_16_CRAFTING_AND_PROFESSIONS.md`
- `docs/source-of-truth/ui/UI_17_MENU.md`
- `docs/source-of-truth/ui/UI_18_WALLET_AND_ECONOMY.md`
- `docs/source-of-truth/ui/UI_19_SETTINGS_AND_SYSTEM_STATES.md`
- `docs/source-of-truth/ui/UI_20_GUILD.md`

---

# 4. Engineering / Project docs

- `README.md` — human-friendly entry point.
- `AGENTS.md` — concise Codex project map and invariants.

- `docs/source-of-truth/architecture/00_DEVELOPMENT_ROADMAP.md`
- `docs/source-of-truth/architecture/00_DEVELOPMENT_STACK.md`
- `docs/source-of-truth/architecture/00_COMPATIBILITY_MATRIX.md`
- `docs/source-of-truth/architecture/00_CONTENT_AND_BALANCE_PROFILES.md`
- `docs/source-of-truth/architecture/00_PRODUCT_AND_PROTOTYPE_STRATEGY.md`
- `docs/source-of-truth/architecture/PHASE_00_ENGINEERING_FOUNDATION_IMPLEMENTATION.md` — completed engineering foundation and verification checklist.
- `docs/source-of-truth/architecture/PHASE_01_TELEGRAM_IDENTITY_WORLD_IMPLEMENTATION.md` — current implementation phase.

---

# 5. UI master docs

- `docs/source-of-truth/ui/00_UI_UX_CONCEPT.md`
- `docs/source-of-truth/ui/00_MASTER_UI_REFERENCE.md`
- `docs/source-of-truth/ui/00_UI_REFERENCE_INDEX.md`
- `docs/source-of-truth/ui/00_UI_PACK_SUMMARY.md`

---

# 6. Historical validation artifacts

- `docs/archive/00_FULL_AUDIT_V7_1.md`
- `docs/archive/00_MANIFEST.md`

These files describe the archived v7.1 package and are not proof that the current checkout passes build, tests, or asset integrity.

---

# 7. Current visual references

- `reference/UI_01-02_GLOBAL_SHELL_WORLD.png`
- `reference/UI_03-04_HERO_INVENTORY.png`
- `reference/UI_05-06_STATS_TALENTS.png`
- `reference/UI_07-08_COMPANION_COMBAT.png`
- `reference/UI_09-10_RAID_PARTY.png`
- `reference/UI_11-12_QUESTS_CITY.png`
- `reference/UI_13-14_MERCHANT_AUCTION.png`
- `reference/UI_15-16_DUNGEON_CRAFTING.png`
- `reference/UI_17-18_MENU_WALLET.png`
- `reference/UI_19-20_SETTINGS_GUILD.png`

---

# 8. Rule

Новые решения не должны тихо перезаписывать старые документы.

Если gameplay decision меняется:
1. обновить system document;
2. обновить dependent UI documents;
3. обновить compatibility/audit;
4. только затем обновлять visual reference.
