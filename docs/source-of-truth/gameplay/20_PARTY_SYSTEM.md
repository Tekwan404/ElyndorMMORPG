...
После успешного создания нового invite сервер делает best-effort попытку отправить приглашённому уведомление через Telegram на `TelegramUserId`, привязанный к его Account.

Telegram notification:
- содержит имя персонажа, который отправил приглашение;
- сообщает, что приглашение действует 5 минут;
- содержит inline Web App кнопку `Принять и войти`, которая передаёт `PartyInviteId` в Elyndor;
- после открытия Mini App клиент ждёт успешной авторизации и состояния `world`, затем один раз вызывает существующий authoritative accept mutation;
- параметр invite удаляется из URL после чтения, чтобы reload не повторял auto-accept;
- истёкший, уже использованный или невалидный invite не блокирует вход игрока в игру;
- является вторичным каналом доставки и не заменяет authoritative invite в Party System;
- не должна откатывать или превращать созданный invite в ошибку, если Telegram API недоступен;
- не должна намеренно отправляться повторно при последовательном idempotent replay того же `PartyInviteId`.
