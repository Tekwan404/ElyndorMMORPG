...
Определения трёх базовых заклинаний существуют в content заранее, но Маг **не получает активные способности автоматически**.

Способность школы появляется у персонажа только после вложения первого очка в начальный талант соответствующей ветки:

- `F-1-1 Точное Пламя` → `MAGE_FIREBALL`;
- `A-1-1 Тайная Точность` → `MAGE_ARCANE_SPARK`;
- `I-1-1 Холодная Точность` → `MAGE_ICE_SHARD`.

Это общее правило Mage tree: новые активные способности открываются талантами и не выдаются персонажу без прокачки.
