...
**Document:** docs/source-of-truth/architecture/00_DEVELOPMENT_STACK.md
...
Optional Cache/Scale:  Redis + StackExchange.Redis, only for a measured need
...
Не требовать локально установленный PostgreSQL. Если текущая фаза обоснованно использует Redis, оба ресурса поднимает Aspire containers. Это уменьшает различия между машинами разработчиков.
...
- EF Core;
- PostgreSQL;
- Redis adapters when a measured Redis use case exists;
- SignalR;
- Telegram;
- ASP.NET HTTP.
...
- PostgreSQL;
- Redis when a measured Redis use case exists;
- Server;
- Vue/Vite;
- environment wiring;
- dashboard.
...
Packages are added in the phase that first uses them. Presence in this list does not make Redis, Quartz, JWT auth, or a test container mandatory for an earlier phase.
...
StackExchange.Redis                                      # only for a measured Redis use case
Quartz                                                   # when durable scheduled jobs begin
Quartz.Extensions.DependencyInjection                    # with Quartz
Quartz.Extensions.Hosting                                # with Quartz
Quartz.Serialization.SystemTextJson                      # with Quartz
...
Aspire.Hosting.Redis                                      # only when Redis is used
...
Testcontainers.Redis                                      # only for Redis-specific integration behavior
```
...
Redis подключаем только при измеримой необходимости и используем осознанно.
...
Redis appears in this topology only after a current feature justifies it.
...
PostgreSQL не нужно устанавливать руками локально — достаточно container runtime. То же относится к Redis, если он позже появится в AppHost.
...
An optional Redis container is added only after a measured need appears.
...
Redis is deployed only if the implemented feature set actually uses it.
...
```text
1. Solution + Aspire + PostgreSQL + Vue
2. Content loader + validator
