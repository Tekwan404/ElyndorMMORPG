...
The page may scroll vertically when two skill rows, safe-area padding, or Telegram chrome make the content taller than the viewport. Actor art and controls must never overlap. Bottom navigation stays hidden during combat.
...
`useBattleFormation.ts` is a pure layout projection. It accepts actor ids, local actor id, aggro actor id, viewport class, and fallback mode, then returns stable slots containing normalized x/y, scale, z-index, and visibility.
