...
function actor(id: string, kind: CombatActorSnapshot['kind'], abilities: CombatAbility[] = []): CombatActorSnapshot {
  return {
    actorId: id,
    kind,
...
function ability(index: number): CombatAbility {
  return { id: `ABILITY_${index}`, displayName: `Skill ${index}`, description: '', iconId: null, resourceCost: 0, cooldownSeconds: 0, targetType: 'SingleEnemy' }
}
...
describe('BattleScreen', () => {
  ...
  it('uses one screen for party formation, skills, controls and collapsed log', () => {
    ...
    const enemy = { ...actor('enemy', 'Monster'), currentAggroTargetActorId: 'ally' }
    store.snapshot = {
      sessionId: 'session', sequence: 1, status: 'Active', serverTimeUtc: '2026-09-25T12:00:00Z',
      contentVersion: 'test', balanceVersion: 'test', player: local, players: [local, ally], enemy,
    }
  ...
  it('switches the server-selected hostile actor without changing friendly selection', async () => {
    ...
    const alpha = { ...actor('alpha', 'Monster'), name: 'Альфа-волк', artId: 'alpha-wolf' }
    store.snapshot = {
      sessionId: 'session', sequence: 1, status: 'Active', serverTimeUtc: '2026-09-25T12:00:00Z',
      contentVersion: 'test', balanceVersion: 'test', player: local, enemy: wolf,
      enemies: [wolf, alpha], selectedTargetActorId: wolf.actorId,
    }
  ...
  it('renders authoritative cast names and unblockable enemy state', () => {
    const store = useCombatSessionStore()
    const fireball = { ...ability(1), id: 'FIREBALL', displayName: 'Огненный шар' }
    const crush = { ...ability(2), id: 'CRUSH', displayName: 'Сокрушение', isUnblockable: true }
    const local = actor('local', 'Player', [fireball])
    const enemy = actor('enemy', 'Monster', [crush])
    local.activeCast = { abilityId: 'FIREBALL', startedAtUtc: new Date(Date.now() - 100).toISOString(), resolvesAtUtc: new Date(Date.now() + 900).toISOString() }
    enemy.activeCast = { abilityId: 'CRUSH', startedAtUtc: new Date(Date.now() - 100).toISOString(), resolvesAtUtc: new Date(Date.now() + 900).toISOString() }
    store.snapshot = {
      sessionId: 'session', sequence: 1, status: 'Active', serverTimeUtc: new Date().toISOString(),
      contentVersion: 'test', balanceVersion: 'test', player: local, enemy,
    }
  ...
  it('keeps consumables separate from skills and keeps server Need eligibility', () => {
    ...
    game.snapshot = { character: { inventory: { items: [consumable()] } } } as never
    store.snapshot = {
      sessionId: 'session', sequence: 1, status: 'Active', serverTimeUtc: '2026-09-25T12:00:00Z',
      contentVersion: 'test', balanceVersion: 'test', player: actor('local', 'Player'), enemy: actor('enemy', 'Monster'),
    }
    store.lootRolls = [{
      lootRollId: 'roll', itemId: 'SWORD', name: 'Меч', rarity: 'Epic', quantity: 1,
      endsAtUtc: '2026-09-25T12:01:00Z', eligibleCharacterIds: [], canNeed: false,
    }]

    const wrapper = mount(BattleScreen)

    expect(wrapper.get('[data-consumable-bar]').find('[data-combat-consumable="HEALING_POTION"]').exists()).toBe(true)
    expect(wrapper.get('[data-combat-hotbar]').find('[data-combat-consumable]').exists()).toBe(false)
...
function consumable(): InventoryItem {
  return {
    id: 'potion-instance', definitionId: 'HEALING_POTION', name: 'Healing Potion', type: 'Consumable', rarity: 'Common',
    ...
    description: '', setId: null, weaponCategory: null, armorCategory: null,
    ...
    isLocked: false, iconId: null, appearanceProfileId: null, weaponBaseAttackIntervalSeconds: null,
    attackSpeedPercent: 0, dodgePercent: 0,
  }
}
