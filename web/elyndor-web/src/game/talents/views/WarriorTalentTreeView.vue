...
const activeBranch = computed(() => warriorTalentBranches.value.find((branch) => branch.id === activeBranchId.value))
const branchTalents = computed(() => warriorTalents.value.filter((talent) => talent.branchId === activeBranchId.value))
...
function talentsInVisualRow(row: number): readonly TalentNode[] {
  const talents = branchTalents.value.filter((talent) => visualRowFor(talent) === row)
  if (!usesCustomVisualLayout.value) return talents
  return [...talents].sort(
...
function isVisualRowLocked(row: number): boolean {
  ...
  const talents = talentsInVisualRow(row)
  if (talents.length === 0) return false
  const minimumRequirement = Math.min(...talents.map((talent) => talent.requiredSpentPoints))
...
function iconFor(talent: TalentNode): IconConfig {
  return {
    ...
    category: 'skill',
    modifier: talent.branchId === 'GUARDIAN' ? 'ice' : talent.branchId === 'BERSERKER' ? 'fire' : 'holy',
...
<template>
  ...
  <section v-else class="talents" data-talent-tree :data-tone="activeBranch.id.toLowerCase()">
    ...
    <nav class="branches" aria-label="Ветки талантов">
      <button v-for="branch in warriorTalentBranches" :key="branch.id" :data-branch="branch.id" type="button"
        :class="{ active: activeBranchId === branch.id }" :aria-current="activeBranchId === branch.id ? 'page' : undefined"
    ...
    <div class="tree" :style="{ height: `${treeHeight}px` }">
      <svg class="tree__connections" :viewBox="`0 0 400 ${treeHeight}`" preserveAspectRatio="none" aria-hidden="true">
        <path v-for="connection in connections" :key="connection.id" :d="connectionPath(connection)" />
      </svg>
      <section v-for="row in tiers" :key="row" class="tier" :data-visual-row="row"
        :class="{ 'tier--locked': isVisualRowLocked(row) }"
        :style="{ top: `${(row - 1) * rowHeight}px`, height: `${rowHeight}px` }">
        ...
        <div class="tier__nodes" :style="{ gridTemplateColumns: visualGridColumns(row) }">
          <button v-for="talent in talentsInVisualRow(row)" :key="talent.id" data-talent-node type="button"
            class="talent-node" :class="`talent-node--${stateFor(talent)}`" :style="talentVisualStyle(talent)"
            :data-gameplay-tier="talent.tier" :data-visual-row="visualRowFor(talent)"
            :aria-label="`${talent.name}, ${rankFor(talent.id)} из ${talent.maxRank}`" @click="selectedTalent = talent">
            <img v-if="artFor(talent)" class="talent-node__art" :src="artFor(talent)!" :alt="talent.name" loading="lazy" decoding="async" />
    ...
    <UIModal :open="selectedTalent !== null" :title="selectedTalent?.name ?? ''" @close="selectedTalent = null">
      ...
      <template #actions>
        <UIButton data-learn-talent :loading="pending" :disabled="!canLearnSelected" @click="learnSelected">Изучить</UIButton>
      </template>
    </UIModal>
  </section>
</template>
...
<style scoped>
.talents { --branch-accent: var(--ui-color-secondary); min-height: 100%; color: var(--ui-color-text-primary); background: radial-gradient(circle at 50% 12rem, rgb(75 84 160 / 16%), transparent 21rem), linear-gradient(180deg, var(--ui-color-surface-1), var(--ui-color-background)); }
.talent-status { padding: var(--ui-space-6) var(--ui-space-4); text-align: center; }
.talent-status--error, .mutation-error { color: var(--ui-color-danger); }
.mutation-error { margin: 0 var(--ui-space-4) var(--ui-space-3); padding: var(--ui-space-2) var(--ui-space-3); border: 1px solid color-mix(in srgb, var(--ui-color-danger) 45%, transparent); border-radius: var(--ui-radius-sm); background: color-mix(in srgb, var(--ui-color-danger) 9%, transparent); font-size: var(--ui-font-size-sm); }
...
.talents__topbar, .branch-intro, .talents__footer { display: flex; align-items: center; justify-content: space-between; gap: var(--ui-space-3); }
.talents__topbar { padding: var(--ui-space-4); }
.eyebrow, .talents h1, .branch-intro h2, .branch-intro p { margin: 0; }
.eyebrow { color: var(--branch-accent); font-size: var(--ui-font-size-xs); letter-spacing: 0.1em; text-transform: uppercase; }
.talents h1, .branch-intro h2 { font-family: var(--ui-font-display); }
.points { display: grid; min-width: 4.25rem; padding: var(--ui-space-2) var(--ui-space-3); border: 1px solid color-mix(in srgb, var(--branch-accent) 46%, transparent); border-radius: var(--ui-radius-md); background: var(--ui-color-surface-2); text-align: center; }
...
.loadouts, .branches { display: grid; border-block: 1px solid var(--ui-color-border); }
.loadouts { grid-template-columns: 1fr 1fr auto; align-items: center; padding: var(--ui-space-2) var(--ui-space-4); background: var(--ui-color-surface-2); }
.loadouts button, .branches button { min-height: var(--ui-touch-target); border: 0; background: transparent; color: var(--ui-color-text-muted); font: inherit; }
.loadouts button.active, .branches button.active { color: var(--ui-color-text-primary); }
.loadouts button.active { border: 1px solid var(--ui-color-primary); border-radius: var(--ui-radius-sm); background: rgb(127 123 234 / 12%); }
.loadouts > small { padding-left: var(--ui-space-2); color: var(--ui-color-text-muted); }
.branches { grid-template-columns: repeat(3, minmax(0, 1fr)); border-top: 0; background: var(--ui-color-surface-1); }
.branches button { position: relative; display: flex; align-items: center; justify-content: center; gap: var(--ui-space-2); }
.branches button.active::after { position: absolute; right: var(--ui-space-3); bottom: -1px; left: var(--ui-space-3); height: 2px; background: var(--branch-accent); box-shadow: 0 0 8px var(--branch-accent); content: ''; }
.branches small { color: var(--branch-accent); }
.branch-intro { padding: var(--ui-space-4); }
.branch-intro span, .branch-intro p, .branch-intro strong small { color: var(--ui-color-text-muted); font-size: var(--ui-font-size-xs); }
.branch-intro strong { color: var(--branch-accent); font-size: var(--ui-font-size-xl); }
.state-legend { display: flex; gap: var(--ui-space-3); padding: 0 var(--ui-space-4) var(--ui-space-3); color: var(--ui-color-text-muted); font-size: var(--ui-font-size-xs); }
...
.tree { position: relative; margin-inline: var(--ui-space-2); overflow: hidden; border: 1px solid var(--ui-color-border); background: linear-gradient(90deg, transparent 49.8%, rgb(127 123 234 / 8%) 50%, transparent 50.2%), rgb(5 8 16 / 66%); }
.tree__connections { position: absolute; z-index: 0; inset: 0; width: 100%; height: 100%; }
.tree__connections path { fill: none; stroke: color-mix(in srgb, var(--branch-accent) 44%, var(--ui-color-border)); stroke-width: 2; vector-effect: non-scaling-stroke; }
.tier { position: absolute; right: 0; left: 0; border-bottom: 1px solid var(--ui-color-border); }
.tier--locked { background: rgb(3 5 10 / 32%); }
.tier__label { position: absolute; z-index: 2; top: var(--ui-space-1); left: var(--ui-space-2); display: grid; margin: 0; color: var(--ui-color-text-muted); font-size: 0.58rem; text-transform: uppercase; }
.tier__label span { font-size: 0.52rem; }
.tier__nodes { position: relative; z-index: 1; display: grid; height: 100%; align-items: center; justify-items: center; padding: var(--ui-space-5) var(--ui-space-1) 0; }
.talent-node { position: relative; width: 3.6rem; height: 3.6rem; padding: 0.3rem; border: 2px solid var(--ui-color-border-strong); border-radius: 50%; background: var(--ui-color-surface-2); color: var(--ui-color-text-muted); box-shadow: 0 0 0 4px rgb(8 12 22 / 82%); }
.talent-node :deep(.icon-generator) { width: 100%; height: 100%; }
.talent-node__art { width: 100%; height: 100%; border-radius: 50%; object-fit: cover; }
...
.talent-detail__art { width: var(--ui-icon-slot-lg); height: var(--ui-icon-slot-lg); flex: 0 0 auto; border: 1px solid var(--branch-accent); border-radius: var(--ui-radius-md); object-fit: cover; box-shadow: 0 0 12px color-mix(in srgb, var(--branch-accent) 32%, transparent); }
...
@media (max-width: 350px) { .talent-node { width: 3.2rem; height: 3.2rem; } .loadouts { grid-template-columns: 1fr 1fr; } .loadouts > small { display: none; } .branches button { gap: var(--ui-space-1); font-size: var(--ui-font-size-sm); } }
</style>