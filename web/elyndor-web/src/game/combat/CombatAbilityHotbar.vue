  ...
  <section class="combat-action-hotbar" data-combat-hotbar>
    ...
    <section v-if="auraAbilities.length" class="combat-ability-hotbar__auras" aria-label="Ауры" data-combat-auras>
      ...
      >
        ...
        <b v-if="cooldownRemaining(ability.id) > 0" :style="cooldownStyle(ability)">
          <span>{{ Math.ceil(cooldownRemaining(ability.id)) }}с</span>
        </b>
      </button>
    </section>
...
.combat-ability-hotbar__slot { position: relative; display: grid; min-width: 0; min-height: clamp(46px, 13vw, 58px); place-items: center; align-content: center; gap: 2px; padding: 3px 2px; border: 1px solid var(--ui-color-border); border-radius: var(--ui-radius-md); background: linear-gradient(180deg, rgb(255 255 255 / 2.5%), rgb(2 5 9 / 45%)); color: var(--ui-color-text-primary); font: inherit; touch-action: manipulation; }
.combat-ability-hotbar__slot[data-state='ready'] { border-color: rgb(146 136 255 / 40%); box-shadow: inset 0 0 0 1px rgb(146 136 255 / 5%); }
.combat-ability-hotbar__slot[data-state='cooldown'], .combat-ability-hotbar__slot[data-state='resource'] { opacity: .5; }
.combat-ability-hotbar__slot--empty { opacity: .13; }
...
.combat-ability-hotbar__cooldown { position: absolute; inset: 3px; display: grid; place-items: center; overflow: hidden; border-radius: 8px; background: conic-gradient(from 0deg, rgb(1 3 7 / 90%) var(--cooldown-progress), rgb(1 3 7 / 42%) 0); color: white; font-size: .78rem; font-variant-numeric: tabular-nums; }
.combat-ability-hotbar__cooldown::after { position: absolute; inset: 20%; border-radius: 50%; background: rgb(2 4 8 / 84%); content: ''; }
...
.combat-consumables { display: grid; gap: 4px; padding-top: 5px; border-top: 1px solid rgb(79 185 150 / 18%); }
.combat-consumables__header { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; padding-inline: 2px; }
.combat-consumables__header strong { color: #b9ddd1; font-size: .5rem; font-weight: 850; letter-spacing: .05em; text-transform: uppercase; }
.combat-consumables__header small { color: var(--ui-color-text-muted); font-size: .44rem; }
.combat-consumables__list { display: flex; gap: 4px; overflow-x: auto; padding-bottom: 1px; scrollbar-width: none; }
...
.combat-consumables__cooldown { position: absolute; inset: 2px; display: grid; place-items: center; border-radius: inherit; background: rgb(1 5 6 / 82%); color: white; font-size: .68rem; }

.combat-ability-hotbar__auras { display: flex; align-items: center; gap: 5px; padding: 5px; border: 1px solid rgb(205 177 113 / 28%); border-radius: var(--ui-radius-sm); background: rgb(205 177 113 / 6%); }
...
.combat-ability-hotbar__aura b { position: absolute; inset: 2px; display: grid; place-items: center; overflow: hidden; border-radius: inherit; background: conic-gradient(from 0deg, rgb(1 3 7 / 88%) var(--cooldown-progress), rgb(1 3 7 / 42%) 0); color: white; font-size: .72rem; }
.combat-ability-hotbar__aura b::after { position: absolute; inset: 22%; border-radius: 50%; background: rgb(2 4 8 / 84%); content: ''; }
.combat-ability-hotbar__aura b > span { position: relative; z-index: 1; }
.combat-ability-hotbar__inspection { display: flex; align-items: flex-start; justify-content: space-between; gap: 10px; padding: 8px 10px; border: 1px solid rgb(146 136 255 / 38%); border-radius: var(--ui-radius-sm); background: rgb(8 11 20 / 96%); box-shadow: 0 5px 14px rgb(0 0 0 / 25%); }
...
@media (max-width: 420px) {
  .combat-ability-hotbar { gap: 3px; }
  .combat-ability-hotbar__slot { min-height: 46px; border-radius: 8px; }
  .combat-ability-hotbar__icon { width: min(34px, 8.6vw); height: min(34px, 8.6vw); border-radius: 7px; }
  .combat-ability-hotbar__slot > small { font-size: .36rem; }
