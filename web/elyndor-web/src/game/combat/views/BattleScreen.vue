...
const displayEnemies = computed(() => activeEnemies.value.length ? activeEnemies.value : battle.enemies.value)
...
const combatConsumables = computed(() => (session.snapshot?.character?.inventory.items ?? [])
  .filter((item) => item.type === 'Consumable' && item.quantity > 0)
  .filter((item) => item.consumableActions.every((action) =>
    action.type !== 'RestoreResource' || action.resourceType === localActor.value?.resourceType,
  )))
...
const enemyCastUnblockable = computed(() => {
  ...
  return cast !== null && selectedEnemy.value?.abilities.some((ability) =>
    ability.id === cast.abilityId && ability.isUnblockable === true,
  ) === true
})
const isActive = computed(() => snapshot.value?.status === 'Active')
const hasParty = computed(() => battle.allies.value.length > 1)
...
function abilityName(abilityId: string): string {
  const ability = [...(localActor.value?.abilities ?? []), ...(selectedEnemy.value?.abilities ?? [])]
...
function rarityLabel(rarity: string): string {
  return ({ Common: 'Обычная', Uncommon: 'Необычная', Rare: 'Редкая', Epic: 'Эпическая', Legendary: 'Легендарная', Unique: 'Уникальная' } as Record<string, string>)[rarity] ?? 'Ценная'
}
...
<template>
  ...
  >
    <template v-if="snapshot && localActor && selectedEnemy">
      ...
      <section v-if="battle.isAwaitingAttachment.value" class="battle-screen__join" data-combat-join>
        <div><strong>Бой уже идёт</strong><small>Войдите, чтобы присоединиться к группе.</small></div>
        <UIButton :disabled="battle.pending.value" @click="battle.attachCombat(snapshot.sessionId)">Войти в бой</UIButton>
      </section>
      ...
      <div class="battle-screen__arena-wrap">
        ...
        <div v-if="enemyCast" class="battle-cast battle-cast--enemy" :class="{ 'battle-cast--unblockable': enemyCastUnblockable }" data-enemy-cast>
          <div><strong>{{ abilityName(enemyCast.abilityId) }}</strong><small>{{ enemyCastUnblockable ? 'НЕБЛОКИРУЕМО · ' : '' }}{{ castRemaining(enemyCast).toFixed(1) }}с</small></div>
          <i><span :style="{ width: `${castProgress(enemyCast)}%` }" /></i>
        </div>
      </div>

      <section v-if="battle.isTraining.value" class="battle-screen__training" aria-label="Статистика тренировки">
        <div><small>Время</small><b>{{ trainingElapsedSeconds.toFixed(1) }}с</b></div>
        <div><small>Урон/с</small><b>{{ Math.round(trainingDps).toLocaleString('ru-RU') }}</b></div>
        <div><small>Урон</small><b>{{ Math.round(battle.trainingStats.value.totalDamage).toLocaleString('ru-RU') }}</b></div>
        <div><small>Криты</small><b>{{ battle.trainingStats.value.criticalHits }}</b></div>
      </section>

      <section v-if="battle.isParticipantActive.value && isActive" class="battle-screen__actions">
        <CombatEffectStrip v-if="localActor.effects.length" :effects="localActor.effects" :now="now" side="player" />
      ...
      <section v-if="!isActive" class="battle-screen__result" :data-result="snapshot.status">
        <strong>{{ snapshot.status === 'Victory' ? 'Победа' : snapshot.status === 'Defeat' ? 'Поражение' : 'Бой завершён' }}</strong>
        <span v-if="battle.reward.value">Награда получена</span>
        <UIButton @click="leaveBattle">Вернуться в мир</UIButton>
      </section>

      <section v-if="battle.lootRolls.value.length" class="battle-screen__loot" data-loot-rolls aria-label="Розыгрыш добычи">
        <article v-for="roll in battle.lootRolls.value" :key="roll.lootRollId">
          <ItemIcon :icon-id="roll.iconId" :item-id="roll.itemId" :name="roll.name" :type="roll.type ?? 'Equipment'" :rarity="roll.rarity" />
          <div><small>{{ rarityLabel(roll.rarity) }}</small><strong>{{ roll.name }}<span v-if="roll.quantity > 1"> ×{{ roll.quantity }}</span></strong></div>
          <div class="loot-roll__actions">
            <button type="button" :disabled="battle.pending.value || !roll.canNeed" @click="battle.chooseLootRoll(roll.lootRollId, 'Need')">Нужно</button>
            <button type="button" :disabled="battle.pending.value" @click="battle.chooseLootRoll(roll.lootRollId, 'Greed')">Претендовать</button>
            <button type="button" :disabled="battle.pending.value" @click="battle.chooseLootRoll(roll.lootRollId, 'Pass')">Отказаться</button>
          </div>
        </article>
      </section>

      <CombatLog :entries="battle.eventProjection.value.logEntries" />
      <p v-if="battle.errorCode.value" class="battle-screen__error" role="alert">Не удалось выполнить действие · {{ battle.errorCode.value }}</p>
      ...
      <UIModal :open="fleeConfirmationOpen" title="Сбежать из боя?" @close="fleeConfirmationOpen = false">
        ...
        <template #actions>
          ...
          <UIButton variant="danger" :loading="battle.pending.value" @click="fleeBattle">Сбежать</UIButton>
        </template>
      </UIModal>
    </template>
...
<style scoped>
.battle-screen {
  --battle-gap: clamp(.28rem, 1vw, .46rem);
  display: grid;
  width: min(100%, 62rem);
  height: 100svh;
  min-height: 40rem;
  ...
  gap: var(--battle-gap);
  margin-inline: auto;
  padding: max(.35rem, env(safe-area-inset-top)) max(.4rem, env(safe-area-inset-right)) max(.35rem, env(safe-area-inset-bottom)) max(.4rem, env(safe-area-inset-left));
  overflow: hidden;
  background: radial-gradient(circle at 50% 18%, rgb(82 59 119 / 13%), transparent 26rem), #05070c;
  color: #eee6da;
}
.battle-screen--scroll-fallback { height: auto; min-height: 100svh; overflow-y: auto; }
.battle-screen__arena-wrap { position: relative; min-height: 0; }
.battle-screen__arena-wrap :deep(.battle-arena) { height: 100%; min-height: 0; }
.battle-screen__actions { display: grid; gap: .3rem; }
.battle-screen__join, .battle-screen__result { display: flex; align-items: center; justify-content: space-between; gap: .5rem; padding: .5rem; border: 1px solid rgb(182 153 87 / 35%); border-radius: 8px; background: #0c1018; }
.battle-screen__join div { display: grid; }
.battle-screen__join small, .battle-screen__result span { color: #99938b; font-size: .52rem; }
.battle-screen__training { display: grid; grid-template-columns: repeat(4, 1fr); gap: .25rem; }
.battle-screen__training div { display: grid; gap: .1rem; padding: .32rem; border: 1px solid rgb(151 124 201 / 24%); border-radius: 6px; background: #0b0e16; text-align: center; }
.battle-screen__training small { color: #8e8991; font-size: .42rem; }
.battle-screen__training b { font-size: .64rem; }
.battle-cast { display: grid; gap: .2rem; padding: .3rem .45rem; border: 1px solid rgb(126 105 190 / 42%); border-radius: 7px; background: rgb(7 9 16 / 94%); }
.battle-cast--enemy { position: absolute; z-index: 90; right: 6%; bottom: 1.5rem; left: 48%; border-color: rgb(213 78 99 / 50%); }
.battle-cast--unblockable { border-color: #ff536d; box-shadow: 0 0 16px rgb(224 53 79 / 28%); }
.battle-cast div { display: flex; justify-content: space-between; gap: .5rem; }
.battle-cast strong { overflow: hidden; font-size: .56rem; text-overflow: ellipsis; white-space: nowrap; }
.battle-cast small { color: #aaa4a0; font-size: .48rem; }
.battle-cast > i { display: block; height: 4px; overflow: hidden; border-radius: 999px; background: #08090d; }
.battle-cast > i span { display: block; height: 100%; background: linear-gradient(90deg, #6553ba, #b497f0); }
.battle-cast--enemy > i span { background: linear-gradient(90deg, #9d3148, #eb7182); }
.battle-screen__loot { display: grid; gap: .35rem; padding: .45rem; border: 1px solid rgb(194 155 69 / 36%); border-radius: 8px; background: #0d1017; }
.battle-screen__loot article { display: grid; grid-template-columns: 44px minmax(0, 1fr); gap: .4rem; align-items: center; }
.battle-screen__loot :deep(.item-icon) { width: 44px; height: 44px; }
.battle-screen__loot article > div { display: grid; }
.battle-screen__loot article small { color: #cbb16b; font-size: .45rem; }
.battle-screen__loot article strong { font-size: .62rem; }
.loot-roll__actions { display: grid !important; grid-column: 1 / -1; grid-template-columns: repeat(3, 1fr); gap: .25rem; }
.loot-roll__actions button { min-height: 40px; border: 1px solid rgb(190 159 87 / 30%); border-radius: 6px; background: #090c12; color: #ddd5ca; font: inherit; font-size: .5rem; }
.battle-screen__error { margin: 0; padding: .35rem; border: 1px solid rgb(209 75 93 / 42%); border-radius: 6px; color: #f1a0aa; font-size: .54rem; }
.battle-screen__missing { display: grid; min-height: 50svh; place-content: center; gap: 1rem; text-align: center; }

@media (max-width: 390px) and (min-height: 760px) {
  .battle-screen:not(.battle-screen--scroll-fallback) :deep(.skill-panel__header) { display: none; }
  .battle-screen:not(.battle-screen--scroll-fallback) :deep(.skill-panel) { padding: .3rem; }
  .battle-screen:not(.battle-screen--scroll-fallback) :deep(.skill-panel__ability) { min-height: 64px; }
}
@media (max-height: 740px) {
  .battle-screen { height: auto; min-height: 100svh; overflow-y: auto; }
}
</style>
