...
.combat-missing {
  ...
  background: linear-gradient(rgb(7 9 19 / 55%), rgb(7 9 19 / 94%)), url('@/assets/world/forest.jpg') center/cover;
}
...
.enemy-stage {
  ...
  background:
    linear-gradient(180deg, rgb(7 9 18 / 24%), rgb(7 9 18 / 92%)),
    url('@/assets/world/forest.jpg') center/cover;
}
.enemy-stage__heading {
  z-index: 1;
  display: flex;
  align-items: center;
...
.enemy-stage h1 {
  ...
  font: var(--ui-font-weight-bold) var(--ui-font-size-xl) var(--ui-font-display);
}
.combat-state {
  flex: 0 0 auto;
  padding: 3px 8px;
...
.enemy-portrait__glow {
  position: absolute;
  width: 13rem;
  height: 8rem;
  border-radius: 50%;
  background: rgb(255 134 52 / 14%);
  filter: blur(34px);
}
.enemy-portrait img {
  position: relative;
  ...
  max-height: 14rem;
  object-fit: contain;
  filter: drop-shadow(0 18px 20px rgb(0 0 0 / 62%));
}
.enemy-health {
  z-index: 1;
  display: grid;
  ...
  background: rgb(8 11 23 / 82%);
  backdrop-filter: blur(6px);
}
.enemy-health__numbers {
  display: flex;
  justify-content: space-between;
  gap: var(--ui-space-3);
...
.player-panel, .combat-log {
  padding: var(--ui-space-3) var(--ui-space-4);
  border-bottom: 1px solid var(--ui-color-border);
  background: rgb(15 18 34 / 94%);
}
...
.danger-badge {
  padding: 3px 7px;
  border: 1px solid rgb(255 78 99 / 50%);
  border-radius: var(--ui-radius-round);
  background: rgb(255 78 99 / 10%);
