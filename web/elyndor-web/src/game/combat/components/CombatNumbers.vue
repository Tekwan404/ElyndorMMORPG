...
const visibleEntries = computed(() =>
  props.entries.filter((entry) => entry.targetActorId === props.actorId).slice(-4),
)
...
<template>
  <span
    class="combat-numbers"
    :data-combat-numbers-for="actorId"
    aria-live="polite"
    aria-atomic="false"
  >
...
.combat-number-enter-active {
  animation: combat-number-rise 850ms ease-out both;
}
...
@keyframes combat-number-rise {
  ...
  to {
    opacity: 0;
    transform: translateY(-2.4rem) scale(1.05);
  }
}
