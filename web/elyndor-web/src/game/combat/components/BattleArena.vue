...
const primaryEnemy = computed(() => props.enemies.find((enemy) =>
  enemy.actorId === props.selectedEnemyActorId,
) ?? props.enemies[0] ?? null)
const primaryAggroActorId = computed(() => primaryEnemy.value?.currentAggroTargetActorId
  ?? props.aggroActorIds[0]
  ?? props.localActorId)
const formation = computed(() => buildBattleFormation({
...
const backgroundStyle = computed(() => props.battlefieldArt
...
const enemyHealthRatio = computed(() => primaryEnemy.value && primaryEnemy.value.maxHp > 0
  ...
  >
    ...
    <nav v-if="enemies.length > 1" class="battle-arena__enemy-strip" aria-label="Противники">
      ...
      >
        ...
        <i><b :style="{ width: `${enemy.maxHp > 0 ? enemy.hp / enemy.maxHp * 100 : 0}%` }" /></i>
      </button>
    </nav>
    ...
    >
      <span class="battle-arena__enemy-art">
        <img v-if="enemyArt" :src="enemyArt" :alt="primaryEnemy.name" loading="eager" fetchpriority="high">
...
.battle-arena__vignette { position: absolute; z-index: 0; inset: 0; background: linear-gradient(90deg, rgb(4 5 9 / 8%) 0 55%, rgb(2 4 8 / 18%) 70%, rgb(2 3 7 / 48%)), linear-gradient(0deg, rgb(2 4 7 / 64%), transparent 35%); pointer-events: none; }
.battle-arena__allies { position: absolute; z-index: 2; inset: 0; }
.battle-arena__ally-slot { position: absolute; transform: scale(var(--actor-scale)); transform-origin: center bottom; transition: top 260ms ease-out, left 260ms ease-out, width 260ms ease-out, height 260ms ease-out, transform 260ms ease-out; }
.battle-arena__enemy { position: absolute; z-index: 40; right: 1.5%; bottom: 4%; display: grid; width: 34%; height: 68%; min-width: 44px; min-height: 44px; grid-template-rows: minmax(0, 1fr) auto; padding: 0; border: 0; background: transparent; color: #f6eee2; font: inherit; cursor: pointer; touch-action: manipulation; }
.battle-arena__enemy[data-enemy-scale='elite'] { width: 37%; height: 72%; }
.battle-arena__enemy[data-enemy-scale='boss'] { width: 41%; height: 76%; }
.battle-arena__enemy:focus-visible { outline: 2px solid #df7182; outline-offset: 3px; border-radius: 12px; }
.battle-arena__enemy-art { display: grid; min-height: 0; place-items: end center; }
.battle-arena__enemy-art img { display: block; width: 100%; height: 100%; object-fit: contain; object-position: center bottom; filter: drop-shadow(0 .65rem .55rem rgb(0 0 0 / 85%)); }
.battle-arena__enemy-art > b { align-self: center; font-family: var(--ui-font-display); font-size: 4rem; }
.battle-arena__enemy-caption { display: grid; gap: 2px; padding: .28rem .45rem; border: 1px solid rgb(207 79 99 / 42%); border-radius: 7px; background: rgb(7 8 13 / 88%); box-shadow: 0 0 14px rgb(157 34 52 / 16%); }
.battle-arena__enemy-caption strong { overflow: hidden; font-size: clamp(.58rem, 2.6vw, .78rem); text-overflow: ellipsis; white-space: nowrap; }
.battle-arena__enemy-caption i, .battle-arena__enemy-strip i { display: block; height: 4px; overflow: hidden; border-radius: 999px; background: #14080c; }
.battle-arena__enemy-caption i b, .battle-arena__enemy-strip i b { display: block; height: 100%; background: linear-gradient(90deg, #8f3044, #df6677); }
.battle-arena__enemy-caption small { color: #b9afa3; font-size: .48rem; }
.battle-arena__enemy-strip { position: absolute; z-index: 70; top: .45rem; right: .45rem; display: flex; max-width: 52%; gap: .3rem; }
...
@media (prefers-reduced-motion: reduce) {
  .battle-arena__ally-slot { transition-duration: 1ms; }
}
</style>
