...
const latestEntry = computed(() => props.entries[props.entries.length - 1] ?? null)
...
<template>
  <section class="combat-log" data-combat-log>
    ...
    >
      ...
      <span v-if="latestEntry"
        ><i aria-hidden="true">{{ icon(latestEntry) }}</i
        >{{ latestEntry.text }}</span
      >
      ...
      <div v-if="open" class="combat-log__overlay" @click.self="closeDrawer">
        ...
        >
          <header>
            ...
            <button ref="close" type="button" aria-label="Закрыть журнал боя" @click="closeDrawer">
              ×
            </button>
          </header>
          <ol>
...
.combat-log__toggle {
  ...
  border: 1px solid rgb(177 151 91 / 28%);
  border-radius: 8px;
  background: rgb(5 8 13 / 94%);
  color: #dfd7ca;
  font: inherit;
  text-align: left;
}
.combat-log__toggle strong {
  color: #cfc2a4;
...
.combat-log__drawer {
  ...
  overflow: hidden;
  border: 1px solid rgb(190 158 87 / 52%);
  border-radius: 12px 12px 8px 8px;
  background: linear-gradient(180deg, #11151e, #07090f);
  box-shadow: 0 -12px 38px rgb(0 0 0 / 58%);
}
.combat-log__drawer header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 0.65rem 0.75rem;
  border-bottom: 1px solid rgb(177 151 91 / 22%);
}
.combat-log__drawer h2 {
  margin: 0;
  color: #e7d8b4;
  font-family: var(--ui-font-display);
  font-size: 0.82rem;
}
.combat-log__drawer header button {
  width: 36px;
  height: 36px;
...
.combat-log__drawer li time {
  color: #777a80;
  font-variant-numeric: tabular-nums;
}
.combat-log__drawer li i {
  color: #b99adc;
  font-style: normal;
}
.combat-log__drawer li[data-side='enemy'] i,
.combat-log__drawer li[data-side='enemy'] span {
  color: #e98a98;
}
.combat-log__drawer li[data-side='player'] span {
  color: #e8d8b0;
}
