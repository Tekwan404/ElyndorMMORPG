...
.allies-strip__actor {
  ...
  padding: 2px;
  overflow: hidden;
  border: 1px solid rgb(177 157 105 / 28%);
  border-radius: 8px;
  background: rgb(6 8 13 / 78%);
  color: #e8dfd0;
  cursor: pointer;
  touch-action: manipulation;
}
.allies-strip__actor[data-selected='true'] {
  border-color: #e4c66f;
  box-shadow:
    0 0 0 1px rgb(228 198 111 / 20%),
    0 0 10px rgb(228 198 111 / 26%);
}
.allies-strip__actor[data-aggro='true'] {
  background: linear-gradient(180deg, rgb(66 17 26 / 82%), rgb(7 8 13 / 90%));
}
.allies-strip__actor:focus-visible {
  outline: 2px solid #f0d57f;
  outline-offset: 2px;
}
.allies-strip__portrait {
  display: grid;
  min-height: 0;
  place-items: end center;
  overflow: hidden;
}
.allies-strip__portrait img {
  width: 100%;
  height: 100%;
  object-fit: contain;
  object-position: center 18%;
}
.allies-strip__health {
  display: block;
  overflow: hidden;
  border-radius: 999px;
  background: #160b0e;
}
.allies-strip__health i {
  display: block;
  height: 100%;
  background: #bd5065;
}
.allies-strip__actor > small {
  position: absolute;
  top: 2px;
  right: 3px;
  color: #ff8d9a;
