    ...
    TimeProvider timeProvider)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        CombatActorState enemyActor = new(
            Guid.NewGuid(), monster.MaxHp, monster.MaxHp, 0, 0, monster.Stats);
        CombatParticipantDefinition player = new(
            playerActor, CombatActorKind.Player, character.ClassId, character.Name,
            character.Vitals.ResourceType, classProfile.CombatAutoAttack,
            new HashSet<string>(character.KnownAbilityIds, StringComparer.Ordinal));
        CombatParticipantDefinition enemy = new(
            enemyActor, CombatActorKind.Monster, monster.Id, monster.Name, "NONE",
        ...
        CombatSession session = new(
            Guid.NewGuid(), player, enemy, abilities, ai, talentModifiers,
            randomFactory.Create(), timeProvider.GetUtcNow());
    ...
    private static CombatSessionCreationResult Failure(string code, Guid characterId = default) =>
