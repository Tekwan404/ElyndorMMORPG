...
public static class TalentEndpoints
{
    ...
    private static async Task<IResult> GetAsync(
        ClaimsPrincipal user, HttpContext context, TalentService service, CancellationToken cancellationToken)
    {
    ...
    private static async Task<IResult> LearnAsync(
        LearnTalentRequest request, ClaimsPrincipal user, HttpContext context,
        TalentService service, CancellationToken cancellationToken)
    {
        return TryGetAccountId(user, out Guid accountId)
    ...
    private static async Task<IResult> SwitchAsync(
        SwitchTalentLoadoutRequest request, ClaimsPrincipal user, HttpContext context,
        TalentService service, CancellationToken cancellationToken)
    {
        return TryGetAccountId(user, out Guid accountId)
    ...
    private static async Task<IResult> ResetAsync(
        ResetTalentLoadoutRequest request, ClaimsPrincipal user, HttpContext context,
        TalentService service, CancellationToken cancellationToken)
    {
        return TryGetAccountId(user, out Guid accountId)
    ...
    {
        IReadOnlyDictionary<string, int> activeRanks = snapshot.State.GetRanks(snapshot.State.ActiveLoadoutId);
        int earned = TalentRules.EarnedPoints(snapshot.Character.Level, snapshot.Tree.MaxSpendablePoints);
        return new TalentSnapshotResponse(
            snapshot.Tree.Id, snapshot.Tree.ClassId, snapshot.Tree.Version,
            snapshot.State.ActiveLoadoutId, snapshot.State.StateVersion, earned,
            Math.Max(0, earned - activeRanks.Values.Sum()),
            snapshot.Tree.Branches.Select(branch => new TalentBranchResponse(
                branch.Id, branch.Name, branch.Fantasy, branch.NodeCount)).ToArray(),
            snapshot.Tree.Nodes.Select(node => new TalentNodeResponse(
                node.Id, node.BranchId, node.Tier, node.RequiredSpentPoints, node.Name,
            ...
            [
                ...
                new TalentLoadoutResponse(TalentLoadoutIds.Loadout2, snapshot.Loadout2Ranks,
